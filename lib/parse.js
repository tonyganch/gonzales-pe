...
function parse(css, options) {
    ...
    try {
        ...
        ast.traverseByType = traverseByType.bind(ast);
        ast.traverseByTypes = traverseByTypes.bind(ast);
    } catch (e) {
...
function buildIndex(ast, index, indexHasChanged) {
    ...
    for (var i = 0, l = ast.content.length; i < l; i++) {
        ...
        buildIndex(node, index);
    }
}

function traverseByType(type, callback) {
    if (!this.index || this.indexHasChanged && this.indexHasChanged[0]) {
