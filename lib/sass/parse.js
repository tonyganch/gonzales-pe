...
module.exports = (function() {
    ...
    function getLastPositionForString(content, line, column, colOffset) {
        ...
        // Line:
        var offset = newline === -1 ? 0 : content.split('\n').length - 1;
        position[0] = line + offset;

        // Column:
        offset = newline === -1 ?
            content.length - 1 : content.slice(newline).length - 2;
...
/////////////////////////////////////
    ...
    function checkAny(i) {
        ...
        else if (l = checkFunctionExpression(i)) tokens[i].any_child = 11;
    ...
    function getAny() {
        ...
        else if (childType === 11) return getFunctionExpression();
    ...
    */
    ...
    function getFilter() {
        ...
        var token = tokens[startPos];
        return newNode(NodeType.FilterType, x, token.ln, token.col);
    }
    ...
    function getFilterp() {
        var startPos = pos,
            token = tokens[pos],
            ident = newNode(NodeType.IdentType, joinValues2(pos, tokens[pos].filterp_l), token.ln, token.col),
            x;
    ...
    function getFilterv() {
        ...
        if (checkDeclDelim(pos))
            return newNode(NodeType.FiltervType, x, token.ln, token.col);
    ...
    function getFunctionExpression() {
        ...
        return newNode(NodeType.FunctionExpressionType, x, token.ln, token.col, end);
    }
    ...
    function getImportant() {
        ...
        if (!x || !x.length) x = '';
        var offset = x ? 9 : 10;
        var end = getLastPosition(x, line, column, offset);
    ...
    */
    ...
    function _getProgid(progid_end) {
        ...
        var token = tokens[startPos];
        return newNode(NodeType.RawType, x, token.ln, token.col);
    }
